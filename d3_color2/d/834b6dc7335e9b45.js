  ...
  // control that shows state info on hover
	...
	infoControl.update = function (feat) {
			this._div.innerHTML = '<h5>Número de homicídios</h5>' +  (feat ?
				'<b>' + feat.properties.NOME + '</b><br />' + homicidesByName.get(feat.properties.NOME)[0] + ' homicídios.<br/>' +
        homicidesByName.get(feat.properties.NOME)[1] + ' homicídios/100.000 habitantes.'
    ...
    data.forEach(function(d) {
      let homicidiosPorPopulacao;
        if(d.Populacao == -1)
          ...
          homicidiosPorPopulacao = ((d.Homicidios/d.Populacao) *100000).toFixed(2)
            bairrosMap.set(d.Bairro,[+d.Homicidios, homicidiosPorPopulacao])
        })
    return bairrosMap
  })
)});
